...
import { clearMarkers } from './contexts/MarkerManager';
  ...
  function SendQuestion() {
    ...
    fetch("http://localhost:3002/api/chat", {
      ...
      .then((data) => {
        ...
        // Dialogflow의 응답 메시지 먼저 추가
        addMessage("chang", data.answer);
        ...
        if (data.parameters && data.parameters.fields) {
          ...
          // dong 값이 있으면 처리
          if (dongValue) {
            const cleanDong = dongValue.replace('동', '') + '동';
            setDongState(cleanDong);
            onDongUpdate && onDongUpdate(cleanDong);
            // 동 선택 시 발생하는 모든 동작을 여기서 처리
            setHoveredMain(null);
          ...
          // 부동산 정보 메시지 추가 (모든 값이 있을 때만)
          if (dongValue && industryValue && payValue) {
            const estateMessage = `부동산 정보를 확인해주세요!\n선택된 동: ${dongValue}\n월세: ${payValue} 미만\n업종: ${industryValue}`;
            addMessage("chang", estateMessage);
          }
        }
      })
      .catch((err) => {
        console.log("에러발생", err);
        addMessage("chang", "죄송합니다. 오류가 발생했습니다. 다시 시도해주세요.");
      });
