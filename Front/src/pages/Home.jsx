...
const Home = () => {
    ...
    const handleDongUpdate = (newDong) => {
        setSelectedDong(newDong);
        setSelectedRegion(newDong);
        setMapToRegion(newDong);
        setPolygonCoordinates(null);
        // DetailPage 컴포넌트의 isOpen 상태를 true로 설정
        const detailPageButton = document.querySelector('.detail-page-toggle');
        if (detailPageButton && !detailPageButton.classList.contains('active')) {
            detailPageButton.click();
        }
    };

    const handlePayUpdate = (newPay) => {
        // 필요한 경우 월세 관련 상태 업데이트
        // console.log('월세 업데이트:', newPay);
    };

    const handleIndustryUpdate = (newIndustry) => {
        setSelectedCategory(newIndustry);
        setMapToCate(newIndustry);
        // RealEstate 컴포넌트의 isOpen 상태를 true로 설정
        const realEstateButton = document.querySelector('.real-estate-toggle');
        if (realEstateButton && !realEstateButton.classList.contains('active')) {
            realEstateButton.click();
        }
    };

    const handleDongChange = (newDong) => {
        setSelectedDong(newDong);
    };
        ...
        <div className="home-container">
            ...
            {(isChatVisible || isClosing) && (
                <div className={`chat-container ${isClosing ? 'hide' : 'show'}`}>
                    <ChatBot 
                        selectedDong={selectedDong}
                        onDongUpdate={handleDongUpdate}
                        onPayUpdate={handlePayUpdate}
